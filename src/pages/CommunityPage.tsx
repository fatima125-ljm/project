...
export function CommunityPage() {
  ...
  const like = async (post: Post) => {
    ...
    if (post.liked_by_me) {
      setPosts((ps) =>
        ps.map((p) => (p.id === post.id ? { ...p, liked_by_me: false, like_count: p.like_count - 1 } : p)),
      );
      await supabase.from('likes').delete().eq('post_id', post.id).eq('user_id', user.id);
    } else {
      setPosts((ps) =>
        ps.map((p) => (p.id === post.id ? { ...p, liked_by_me: true, like_count: p.like_count + 1 } : p)),
      );
  ...
  const comment = async (post: Post) => {
    ...
    if (data) {
      setPosts((ps) =>
        ps.map((p) => (p.id === post.id ? { ...p, comments: [...p.comments, data as Comment] } : p)),
      );
  ...
  const deleteComment = async (post: Post, c: Comment) => {
    ...
    setPosts((ps) =>
      ps.map((p) => (p.id === post.id ? { ...p, comments: p.comments.filter((x) => x.id !== c.id) } : p)),
    );
  };
          ...
          {loading ? (
            ...
            posts.map((post) => (
              <article key={post.id} className="surface overflow-hidden">
                <img src={post.image_url} alt={post.title} className="max-h-96 w-full object-cover" />
                <div className="p-5">
